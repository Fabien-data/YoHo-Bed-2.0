...
export const floorLayoutSchema = z.object({
  ...
  rooms: z.array(z.object({ unitId: z.string().uuid(), x: z.number().int().min(0).max(1000), y: z.number().int().min(0).max(1000) })).max(1000),
  landmarks: z.array(z.object({ id: z.string().min(1).max(50), kind: z.enum(['corridor', 'lift', 'stairs', 'service']), x: z.number().int().min(0).max(1000), y: z.number().int().min(0).max(1000), label: z.string().max(50).optional() })).max(100),
});
...
export const roomSignalsSchema = z.object({ doNotDisturb: z.boolean().optional(), requestedSafetyFlag: z.boolean().optional() })
  .refine(v => Object.keys(v).length > 0, { message: 'nothing to update' });
