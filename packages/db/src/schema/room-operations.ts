...
export const floorLayouts = pgTable(
  ...
  (t) => ({
    propertyFloorUnique: unique('floor_layouts_property_floor_uq').on(t.propertyId, t.floor),
  }),
);

/** Stay-scoped operational signals. Safety is deliberately explicit and access-controlled. */
export const roomStaySignals = pgTable(
  'room_stay_signals',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id')
...
export const housekeepingTasks = pgTable(
  ...
  (t) => ({
    roomDateKindUnique: unique('housekeeping_tasks_room_date_kind_uq').on(
      t.roomUnitId,
      t.date,
      t.kind,
    ),
    ...
    kindValid: check(
      'housekeeping_tasks_kind_valid',
      sql`${t.kind} in ('departure', 'stayover', 'arrival_prep')`,
    ),
    statusValid: check(
      'housekeeping_tasks_status_valid',
      sql`${t.status} in ('queued', 'in_progress', 'done', 'cancelled')`,
    ),
  }),
);
...
export const roomMoves = pgTable(
  'room_moves',
  {
    ...
    fromRoomUnitId: uuid('from_room_unit_id')
      .notNull()
      .references(() => roomUnits.id),
    toRoomUnitId: uuid('to_room_unit_id')
      .notNull()
      .references(() => roomUnits.id),
  ...
  (t) => ({
    statusValid: check(
      'room_moves_status_valid',
      sql`${t.status} in ('planned', 'completed', 'stopped')`,
    ),
  }),
);
